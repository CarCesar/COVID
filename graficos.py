...
def grafico2(df,col,nt,logaritmo):
    ...
    ban = alt.Chart(df).mark_image(width=80,x=660,y=288
                                    ).encode(url='flag',opacity = alt.condition(click1,
                                                                  alt.value(1),
                                                                  alt.value(0))
                                            ).properties(width=700,height=400)
    
    return (fig1|(fig2+fig3+text+text0+text1+text2+ban)).configure_axis(grid=False).configure_view(strokeWidth=0)